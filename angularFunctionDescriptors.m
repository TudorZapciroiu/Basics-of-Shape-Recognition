...
for i = 1:m
    % medierea coordonateor pixelilor de pe contur pe o fereastra de
    ...
    % calculul valorii functiei de curbura in punctul i de pe contur
    dx = x2-x1;
    dy = y2-y1;
...
% calculul functiei unghiulare cumulative
G = zeros(1, m);
for i = 2:m
    dA = A(i)-A(i-1);
...
plot(S,G);
axis([0, S(m), -2*pi-1, 1]);
title('Functie unghiulara cumulativa \gamma(s)');
...
title('Functie unghiulara cumulativa normalizata \gamma^*(t)');

% calculul descriptorilor Fourier
a = zeros(1, nc);
b = zeros(1, nc);
...
subplot(235)
bar(a);
title('Descriptori Fourier a_k')

subplot(236)
bar(b)
title('Descriptori Fourier b_k')
